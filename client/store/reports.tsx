...
export function ReportsProvider({ children }: { children: ReactNode }) {
      ...
      if (prev && prev !== r.status) {
        if (r.status === "acknowledged")
          toast({
            ...
            description: "City staff has reviewed your report.",
          });
        if (r.status === "in_progress")
          toast({
            title: "Work in progress",
            description: "A crew has been assigned to your issue.",
          });
        if (r.status === "resolved")
          toast({
            title: "Issue resolved",
            description: "Thanks for helping improve the city!",
          });
      }
  ...
  const addMutation = useMutation({
    ...
    onSuccess: (r) => {
      toast({
        ...
        description: "We'll notify you as it progresses.",
      });
      prevStatuses.current[r.id] = r.status;
  ...
  const addReport = useCallback<ReportsContextValue["addReport"]>(
    async (r) => {
  ...
  const updateStatus = useCallback<ReportsContextValue["updateStatus"]>(
    async (id, status) => {
  ...
  const value = useMemo<ReportsContextValue>(
    () => ({ reports: current, addReport, updateStatus }),
    [current, addReport, updateStatus],
  );
