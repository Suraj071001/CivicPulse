...
export const ReportForm: React.FC = () => {
  ...
  const canSubmit = useMemo(
    () => description.trim().length > 0 || photoUrl || audioUrl,
    [description, photoUrl, audioUrl],
  );
      ...
      <div className="rounded-xl p-4 bg-card border">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Capture photo, add notes or voice
            </div>
          </div>
          <span
            className={cn(
              ...
              urgency === "low" && "bg-emerald-100 text-emerald-700",
            )}
          >
            {urgency}
          </span>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          ...
          <button
            type="button"
            className={cn(
              "rounded-lg p-3 border flex items-center justify-center gap-2",
              recording
        ...
        {recError ? (
          <div className="text-xs text-destructive mt-2">{recError}</div>
        ) : null}
        ...
        {loc ? (
          <div className="mt-2 text-xs text-muted-foreground">
            Location set ({loc.lat.toFixed(4)}, {loc.lng.toFixed(4)}){" "}
