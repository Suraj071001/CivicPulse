...
export default function Admin() {
  const exportCsv = (rows: ReportDTO[]) => {
    ...
    const lines = [
      header.join(","),
      ...flat.map((row) =>
        header.map((h) => JSON.stringify((row as any)[h] ?? "")).join(","),
      ),
    ];
  ...
  const analytics = useQuery({
    queryKey: ["analytics"],
    queryFn: () => import("@/lib/api").then((m) => m.getAnalytics()),
  });

  const updateMut = useMutation({
    mutationFn: ({ id, patch }: { id: string; patch: Partial<ReportDTO> }) =>
      updateReport(id, patch as any),
    ...
    <div className="space-y-4">
      <div className="rounded-xl p-4 bg-card border space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-lg border p-3 bg-background">
            ...
            <div className="text-lg font-semibold">
              {analytics.data?.totals.total ?? 0}
            </div>
          </div>
          <div className="rounded-lg border p-3 bg-background">
            <div className="text-xs text-muted-foreground">Active</div>
            <div className="text-lg font-semibold">
              {analytics.data?.totals.active ?? 0}
            </div>
          </div>
          <div className="rounded-lg border p-3 bg-background">
            <div className="text-xs text-muted-foreground">Resolved</div>
            <div className="text-lg font-semibold">
              {analytics.data?.totals.resolved ?? 0}
            </div>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <Select
            value={status}
            onValueChange={(v) => setStatus(v === "__any" ? "" : (v as any))}
          >
            <SelectTrigger>
          ...
          <Select
            value={category}
            onValueChange={(v) => setCategory(v === "__any" ? "" : (v as any))}
          >
            <SelectTrigger>
          ...
          <Select
            value={urgency}
            onValueChange={(v) => setUrgency(v === "__any" ? "" : (v as any))}
          >
            <SelectTrigger>
          ...
          <Select
            value={department}
            onValueChange={(v) => setDepartment(v === "__any" ? "" : v)}
          >
            <SelectTrigger>
          ...
          <Select
            value={hasLocation}
            onValueChange={(v) => setHasLocation(v === "__any" ? "" : v)}
          >
            <SelectTrigger>
              <SelectValue placeholder="Has location" />
            </SelectTrigger>
            <SelectContent>
              ...
              <tr key={r.id} className="border-t align-top">
                ...
                <td className="p-2">
                  <Select
                    ...
                    onValueChange={(v) =>
                      updateMut.mutate({
                        id: r.id,
                        patch: { status: v as ReportStatus },
                      })
                    }
                  >
                ...
                <td className="p-2">
                  <Select
                    value={r.department}
                    onValueChange={(v) =>
                      updateMut.mutate({ id: r.id, patch: { department: v } })
                    }
                  >
                    <SelectTrigger className="h-8">
                      <SelectValue />
...
function badgeClass(u: ReportUrgency) {
  if (u === "high")
    return "text-xs px-2 py-1 rounded-full capitalize bg-red-100 text-red-700";
  if (u === "medium")
    return "text-xs px-2 py-1 rounded-full capitalize bg-amber-100 text-amber-700";
