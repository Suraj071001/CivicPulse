...
export async function createReport(
  data: Omit<CreateReportRequest, "photoUrl" | "audioUrl"> & {
    photoFile?: File | null;
    audioFile?: File | null;
  },
): Promise<ReportDTO> {
  ...
  if (data.photoFile)
    payload.photoDataUrl = await fileToDataUrl(data.photoFile);
  if (data.audioFile)
    payload.audioDataUrl = await fileToDataUrl(data.audioFile);
  const res = await tryFetch(`/api/reports`, {
    method: "POST",
...
export async function getAnalytics(): Promise<AnalyticsResponse> {
  try {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "getAnalytics: backend unavailable, returning empty analytics",
      e,
    );
    return {
